...
def user_exists(user_id: uuid.UUID, db: Session = Depends(get_db)):
...
async def delete_chat(chat_id: uuid.UUID, user: User = Depends(user_exists), db: Session = Depends(get_db)):
